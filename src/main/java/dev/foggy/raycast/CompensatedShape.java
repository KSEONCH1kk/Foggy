...
final class CompensatedShape {
    ...
    static final CompensatedShape FULL_BLOCK = new CompensatedShape(
    ...
    /** Allocation-heavy world-space boxes used only by operator diagnostics. */
    List<Aabb> worldBoxes(int blockX, int blockY, int blockZ) {
        if (boxes.length == 0) {
            return Collections.emptyList();
        }
        List<Aabb> result = new ArrayList<>(boxes.length / 6);
        for (int offset = 0; offset < boxes.length; offset += 6) {
            result.add(new Aabb(
                    boxes[offset] + blockX, boxes[offset + 1] + blockY, boxes[offset + 2] + blockZ,
                    boxes[offset + 3] + blockX, boxes[offset + 4] + blockY, boxes[offset + 5] + blockZ));
        }
        return result;
    }
    ...
    static final class ShapeHit {
        private final double distanceFraction;
        private final Vector position;
        private final BlockFace face;

        ShapeHit(double distanceFraction, Vector position, BlockFace face) {
            this.distanceFraction = distanceFraction;
            this.position = position;
            this.face = face;
        }

        double distanceFraction() { return distanceFraction; }
        Vector position() { return position; }
        BlockFace face() { return face; }
    }
}
