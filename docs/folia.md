...
- [Paper ownership checks](https://jd.papermc.io/paper/1.21.4/org/bukkit/Bukkit.html)

## Scheduler model

- Each online player owns one `EntityScheduler.runAtFixedRate(..., 1, 1)` task. It follows the
  entity across region merges, splits, teleports and worlds.
...
Paper exposes the same scheduler API. On Paper these operations resolve onto its single tick
thread, so no separate implementation or server detection is used.
...
1. Use a Folia build matching Minecraft 1.21.4 and PacketEvents 2.13.0.
